-- Seed 100 addresses (generate n = 1..100 from two digit sets)
...
SELECT
    ...
    (SELECT name FROM (
                          SELECT 'Oliver' name UNION SELECT 'Noah' UNION SELECT 'Leo' UNION SELECT 'Jack'
                          UNION SELECT 'Henry' UNION SELECT 'William' UNION SELECT 'Charlie' UNION SELECT 'Lucas'
                          UNION SELECT 'Thomas' UNION SELECT 'James'
                      ) fn ORDER BY n % 10 LIMIT 1) AS first_name,
    (SELECT ln FROM (
    SELECT 'Smith' ln UNION SELECT 'Jones' UNION SELECT 'Williams' UNION SELECT 'Brown'
    UNION SELECT 'Taylor' ln UNION SELECT 'Wilson' UNION SELECT 'Anderson' UNION SELECT 'Thompson'
    UNION SELECT 'White'  UNION SELECT 'Martin'
    ) ln ORDER BY (n/10) % 10 LIMIT 1) AS last_name,
